...
ECUTWFC=38.0          # converged value from the previous test

# Grids to test, from low to high density
# Format: "NxNxN" - the script builds "N N N  0 0 0" automatically
...
for n in $KGRID_LIST; do
    ...
    if [ -z "$prev_energy" ]; then
        printf "%-14s %-24s %-14s %-12s %s\n" \
            "${n}x${n}x${n}" "$energy" "---" "---" "(primero)" >&2
        printf "%-14s %-24s %-14s %-12s %s\n" \
            "${n}x${n}x${n}" "$energy" "---" "---" "primero" >> "$RESULTS_FILE"
        prev_energy="$energy"
        continue
    fi
    ...
    if [ "$below" = "SI" ]; then
        ...
        printf "%-14s %-24s %-14s " "${n}x${n}x${n}" "$energy" "$dmry" >&2
    ...
    printf "%-14s %-24s %-14s %-12s %s\n" \
        "${n}x${n}x${n}" "$energy" "$dmry" "$below" "$consec_file" >> "$RESULTS_FILE"
