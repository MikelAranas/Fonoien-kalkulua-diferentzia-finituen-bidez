...
# ── Vectores recíprocos del grafeno ─────────────────────────────────────────
ca = 8.0
...
# ── Leer path.dat (sin cabecera, tres columnas) ──────────────────────────────
q_path = np.loadtxt('path.dat', unpack=False, usecols=(0, 1, 2))
QN = q_path.shape[0]

# ── Convertir a cartesianas y calcular s_path ────────────────────────────────
q_cart = q_path @ B
s_path = np.zeros(QN)
for i in range(1, QN):
    s_path[i] = s_path[i-1] + linalg.norm(q_cart[i] - q_cart[i-1])
print(s_path)
# ── Guardar graphene_bandak_cm.dat ───────────────────────────────────────────
...
# ── Puntos de alta simetría en coordenadas cristalinas ──────────────────────
...
# ── Líneas y etiquetas en puntos de alta simetría ────────────────────────────
for i in range(QN):
    for q in sympoints:
